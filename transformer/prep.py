...
class Outlier_Remover_with_Z_rands(BaseEstimator,TransformerMixin): # using Standard score, 
    def __init__(self,cols=[]): # the same results as with iqr
        self.stats = {}
        self.cols = cols
    ...
    def transform(self,X,y=None):
        ...
        for col in X.columns:
            ...
            try:
                ...
                ser = x.apply(lambda v: formula(v,mean,std))
                bool_series = ((ser <= -3) & (ser >= 3))
                l = sum(bool_series)     
                
                if best_distr == "gamma":
                    a_gamma, scale_gamma = params["a"],params["scale"] #alpha, beta = params["shape"],params["scale"]
                    x[bool_series] = np.random.gamma(a_gamma, scale=scale_gamma, size=l) #[random.gammavariate(alpha, beta) for i in range(l)]
                elif best_distr == "lognorm":
                    mu, sigma = params["loc"],params["scale"]
                    x[bool_series] = np.random.lognormal(mu, sigma, l) #[random.lognormvariate(mu, sigma) for i in range(l)]                   
                    ...
                    lambda_exp = res.x
                    x[bool_series] =  np.random.exponential(lambda_exp, l)  #[random.expovariate(lambda_exp) for i in range(l)]            
                elif best_distr == "norm": 
                    mu, sigma = params["loc"],params["scale"]
                    x[bool_series] = np.random.normal(mu, sigma, l) #[random.gauss(mu, sigma) for i in range(l)]   
                elif best_distr == "uniform": 
                    a,b = params["loc"],params["scale"]
                    x[bool_series] = np.random.uniform(a, b, l)
                else:
                    continue
            except KeyError:
